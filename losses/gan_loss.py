...
class GANLoss(Module):
    ...
    def get_target_tensor(self, preds, is_real_target):
        real_label = jt.var(round(random.uniform(0.8, 1), 2) if self.real_label is None else self.real_label)
        target_tensor = real_label if is_real_target else jt.var(self.fake_label)
